          ...
          <ul className="flex flex-col">
            {dict.trust.langDemo.map((row, i) => (
              <li
                ...
                className={
                  i > 0
                    ? "rule-b grid grid-cols-[2.5rem_7rem_1fr] items-baseline gap-3 pt-3 pb-3 sm:grid-cols-[3rem_8.5rem_1fr]"
                    : "rule-b grid grid-cols-[2.5rem_7rem_1fr] items-baseline gap-3 pb-3 sm:grid-cols-[3rem_8.5rem_1fr]"
                }
              >
                <span className="doc-label-sm border border-sheet-line px-1.5 py-0.5 text-center text-ink-soft">
                  {row.locale}
                </span>
                <span className="doc-label-sm text-ink-soft">{row.label}</span>
                <span className="fill-line pb-1 text-[0.9375rem] font-medium text-ink">
                  {row.value}
                </span>
              </li>
            ))}
