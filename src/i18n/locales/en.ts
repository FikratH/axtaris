export default {
  paywall: {
    upgrade: 'Upgrade',
    upgradeTo: 'Upgrade to {{plan}}',
    maybeLater: 'Maybe later',
    socialProof: 'Built for job seekers and employers in Azerbaijan',
    socialProofSub: 'A faster way to get hired and to hire',
    mostPopular: 'Most popular',
    proTag: 'Pro',
    limit: {
      title: "You've used all your applications today",
      subtitle: 'Upgrade to Pro for more daily applications and priority placement with employers.',
      benefits: [
        'Send up to 10 applications every day',
        'Priority placement — appear above standard applicants',
        'A Pro badge on every application you send',
      ],
    },
    ai: {
      title: 'Write cover letters with AI',
      subtitle: 'Let AI draft a tailored cover letter in seconds — a Pro feature.',
      benefits: [
        'AI-drafted cover letters tailored to each job',
        'Send up to 10 applications every day',
        'Priority placement with employers',
      ],
    },
  },
  checkout: {
    title: 'Checkout',
    paymentDetails: 'Payment details',
    cardNumber: 'Card number',
    cardHolder: 'Cardholder name',
    cardHolderPlaceholder: 'Full name',
    expiry: 'Expiry',
    expires: 'Expires',
    cvv: 'CVV',
    pay: 'Pay {{price}}',
    processing: 'Processing payment…',
    fillDemo: 'Fill demo card',
    simulatedNote: 'Simulated payment — no real charge. Your plan activates instantly.',
    successSub: 'Welcome to {{plan}}!',
    included: "What's included",
    dueToday: 'Due today',
    freeDuringBeta: 'Free during beta',
    paymentComingSoon: 'Card payments (Azericard, m10) are coming soon. During the beta your plan activates instantly at no charge.',
    secureNote: 'Your subscription is managed securely by AxtarIS',
    activate: 'Activate plan',
    successTitle: "You're all set",
    successMessage: '{{plan}} is now active. Enjoy your new benefits!',
  },
  growth: {
    whoViewedYou: 'Who viewed your profile',
    viewersSubtitle: '{{count}} new this week',
  },
  viewers: {
    title: 'Profile views',
    totalViews: 'Total views',
    thisWeek: 'This week',
    upgradeTitle: 'See who viewed you',
    upgradeSubtitle: 'Upgrade to Pro to unlock the companies checking out your profile.',
    upgradeCta: 'Upgrade to Pro',
    recentViewers: 'Recent viewers',
    emptyTitle: 'No profile views yet',
    emptySubtitle: 'Keep your profile complete so employers can find you.',
    aCompany: 'A company',
    justNow: 'Just now',
    hoursAgo: '{{count}}h ago',
    daysAgo: '{{count}}d ago',
  },
  savedSearch: {
    title: 'Saved searches',
    saveThis: 'Save this search',
    deleteTitle: 'Delete saved search?',
    deleteMessage: 'This search will be removed.',
    newMatches: '{{count}} new',
    allJobs: 'All jobs',
    defaultName: 'My search',
    emptyTitle: 'No saved searches',
    emptySubtitle: 'Save a search to get back to it in one tap.',
    browseJobs: 'Browse jobs',
    limitTitle: 'Saved search limit reached',
    savedTitle: 'Search saved',
    savedMessage: 'You can run it any time from Saved searches.',
    viewSaved: 'View saved',
    upgradeTitle: 'Save more searches',
    upgradeSubtitleFree: 'Upgrade to Pro to save searches and get notified about new matches.',
    upgradeSubtitleLimit: "You've reached your plan's saved-search limit. Upgrade for more.",
    upgradeCta: 'Upgrade to Pro',
  },
  featured: {
    badge: 'Featured',
    toggleLabel: 'Feature this vacancy',
    toggleHint: 'Boost this posting to the top of the candidate feed.',
    limitHint: "You've used all {{count}} featured slot(s) on your plan.",
    limitReachedTitle: 'Featured limit reached',
    limitReached: 'Your plan includes {{count}} featured slot(s). Upgrade for more, or unfeature another vacancy.',
    viewPlans: 'View plans',
  },
  talent: {
    tab: 'Talent',
    title: 'Talent search',
    subtitle: 'Find and invite candidates who fit your roles.',
    searchPlaceholder: 'Search by role, skill, or city',
    filters: 'Filters',
    spotlight: 'Spotlight',
    candidate: 'Candidate',
    invite: 'Invite to apply',
    invited: 'Invited',
    matchPercent: '{{percent}}% match',
    invitesLeft: '{{count}} invites left',
    invitesUnlimited: 'Unlimited invites',
    inviteLimitTitle: 'Invite limit reached',
    inviteLimitBody: "You've used all your invites this month. Upgrade for more.",
    needCompany: 'Set up your company profile first.',
    defaultInviteMessage: '{{company}} would like you to apply.',
    emptyTitle: 'No candidates found',
    emptyBody: 'Try adjusting your search or filters.',
    unavailableTitle: 'Talent search is warming up',
    unavailableBody: 'This feature will be available shortly.',
    notFoundTitle: 'Candidate not found',
    notFoundBody: 'This profile is no longer available.',
    experienceCount: '{{count}} experience entries',
    contactPrivate: 'Contact details stay private until the candidate accepts your invite.',
    anEmployer: 'An employer',
  },
  invites: {
    title: 'Invitations',
    emptyTitle: 'No invitations yet',
    emptyBody: "When employers invite you, they'll show up here.",
    accept: 'Accept',
    decline: 'Decline',
    accepted: 'Accepted',
    declined: 'Declined',
  },
  chat: {
    title: 'Chat',
    messages: 'Messages',
    message: 'Message',
    support: 'Support',
    roleApplicant: 'Applicant',
    roleEmployer: 'Employer',
    tapForDetails: 'tap for details',
    aboutVacancy: 'Vacancy',
    viewVacancy: 'View vacancy',
    attachImage: 'Add photo',
    messagePlaceholder: 'Type a message…',
    empty: 'No messages yet. Say hello!',
    noMessages: 'No messages yet',
    emptyListTitle: 'No conversations yet',
    emptyListSubtitle: 'Messages with employers and candidates appear here.',
    report: 'Report user',
    reportTitle: 'Report {{name}}',
    reportMessage: 'Why are you reporting this user?',
    reportReasonSpam: 'Spam',
    reportReasonHarassment: 'Harassment or abuse',
    reportReasonInappropriate: 'Inappropriate content',
    reportReasonOther: 'Other',
    reportSubmitted: 'Report submitted. Our team will review it.',
    block: 'Block user',
    unblock: 'Unblock user',
    blockConfirmTitle: 'Block {{name}}?',
    blockConfirmMessage: "You won't be able to send or receive messages with this user.",
    unblockConfirmTitle: 'Unblock {{name}}?',
    unblockConfirmMessage: "You'll be able to message each other again.",
    blockedBanner: "You've blocked this user. Unblock to send messages.",
  },
  reviewCv: {
    title: 'Review your CV',
    subtitle: 'We pre-filled your profile from your CV. Edit anything, then save.',
    reviewCta: 'Review & autofill',
    confirm: 'Save to profile',
    discard: 'Discard',
    experienceItem: 'Experience',
    educationItem: 'Education',
    languageItem: 'Language',
    noData: "We couldn't read details from this CV.",
    autofillReady: 'We read your CV — review and auto-fill your profile?',
  },
  cv: {
    title: 'CV',
    preview: 'Preview CV',
    openCv: 'Open CV',
    openExternalHint: 'Open the document in your device viewer.',
  },
  common: {
    suggestions: 'Suggestions',
    later: 'Later',
    appName: 'AxtarIS',
    loading: 'Loading...',
    error: 'Something went wrong',
    retry: 'Try again',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    done: 'Done',
    next: 'Next',
    back: 'Back',
    skip: 'Skip',
    search: 'Search',
    filter: 'Filter',
    apply: 'Apply',
    reset: 'Reset',
    close: 'Close',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    ok: 'OK',
    seeAll: 'See all',
    noResults: 'No results found',
    all: 'All',
    comingSoon: 'Coming soon',
    selectPhoto: 'Select photo',
    camera: 'Camera',
    photoLibrary: 'Photo Library',
    changePhoto: 'Change photo',
    notAvailable: 'Not available',
    tagline: 'Premium employment platform for Azerbaijan',
    deleteItemTitle: 'Delete item?',
    deleteItemMessage: 'This item will be removed from your profile.',
    add: 'Add',
    remove: 'Remove',
    replace: 'Replace',
    defaultUserName: 'there',
    today: 'Today',
    justNow: 'Just now',
  },
  permissions: {
    cameraRequired: 'Camera access is needed to take a photo',
    libraryRequired: 'Photo library access is needed to choose a photo',
  },
  onboarding: {
    slide1Title: 'Build your career',
    slide1Desc: 'Find your dream job with Azerbaijan\'s most modern job search platform',
    slide2Title: 'Smart resume',
    slide2Desc: 'Our AI assistant will help you create a professional resume',
    slide3Title: 'Premium vacancies',
    slide3Desc: 'Apply for exclusive positions from top companies',
    slide3CityBadge: 'Baku',
    chooseLanguage: 'Choose your language',
    chooseLanguageDesc: 'You can change this later in settings',
    chooseTheme: 'Choose your theme',
    chooseThemeDesc: 'Select the look that suits you best',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    getStarted: 'Get started',
  },
  roleSelect: {
    title: 'Who are you?',
    subtitle: 'Choose your role to personalize your experience',
    jobSeeker: 'Looking for a job',
    jobSeekerDesc: 'Find jobs, apply, build your career',
    employer: 'Employer',
    employerDesc: 'Post vacancies, manage candidates',
    registerLater: 'Browse without an account',
    registerLaterHint: 'You can register later to apply and save jobs.',
  },
  auth: {
    signIn: 'Sign in',
    selectCountry: 'Select country',
    signUp: 'Sign up',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm password',
    forgotPassword: 'Forgot password?',
    noAccount: 'Don\'t have an account?',
    hasAccount: 'Already have an account?',
    createAccount: 'Create account',
    fullName: 'Full name',
    companyName: 'Company name',
    phone: 'Phone number',
    signInTitle: 'Welcome back',
    signInSubtitle: 'Sign in to your account',
    signUpTitle: 'Create account',
    signUpSubtitle: 'Start a new journey',
    employerSignUpTitle: 'Register your company',
    employerSignUpSubtitle: 'Create a premium employer account',
    otpTitle: 'Verify your email',
    otpSubtitle: 'Enter the 6-digit code sent to {{email}}',
    otpInvalid: 'Invalid code. Please try again.',
    otpResend: 'Resend code',
    otpResendIn: 'Resend in {{seconds}}s',
    resetTitle: 'Reset password',
    resetSubtitle: 'Enter your email address and we\'ll send you a recovery link',
    resetSent: 'Recovery link sent',
    sendResetLink: 'Send recovery link',
    termsPrefix: 'By continuing, you agree to our',
    termsLink: 'Terms of Service',
    termsAnd: 'and',
    privacyLink: 'Privacy Policy',
    termsSuffix: '',
    sessionExpiredTitle: 'Session expired',
    sessionExpiredMessage: 'Please sign in again to continue securely.',
    fullNamePlaceholder: 'John Smith',
  },
  candidate: {
    home: 'Home',
    autofillFromCv: 'Auto-fill from CV',
    parsingCv: 'Reading your CV…',
    activeJobs: '{{count}} active jobs',
    search: 'Search',
    saved: 'Saved',
    applications: 'Applications',
    profile: 'Profile',
    greeting: 'Hello, {{name}}!',
    recommendedJobs: 'Recommended for you',
    recentJobs: 'Recent vacancies',
    topCompanies: 'Top companies',
    searchPlaceholder: 'Job title, company, or keyword...',
    searchResultsCount_one: '{{count}} vacancy',
    searchResultsCount_other: '{{count}} vacancies',
    searchEmptySubtitle: 'Try different keywords or clear the filters to see more roles.',
    filters: 'Filters',
    jobType: 'Job type',
    salary: 'Salary',
    salaryMin: 'Salary (min)',
    salaryMax: 'Salary (max)',
    experience: 'Experience',
    city: 'City',
    fullTime: 'Full-time',
    partTime: 'Part-time',
    remote: 'Remote',
    hybrid: 'Hybrid',
    onsite: 'At workplace',
    internship: 'Internship',
    noSavedJobs: 'No saved jobs',
    noSavedJobsDesc: 'Save jobs you\'re interested in',
    noApplications: 'No applications',
    noApplicationsDesc: 'Start applying to vacancies',
    applyNow: 'Apply now',
    applied: 'Applied',
    saveJob: 'Save',
    unsaveJob: 'Unsave',
    applicationStatus: 'Application status',
    pending: 'Pending',
    reviewed: 'Reviewed',
    shortlisted: 'Shortlisted',
    rejected: 'Rejected',
    accepted: 'Accepted',
    profileCompletion: 'Profile completion',
    completeProfile: 'Complete your profile',
    editProfile: 'Edit profile',
    uploadCV: 'Upload CV',
    buildResume: 'Build resume',
    aiAssistant: 'AI Assistant',
    skills: 'Skills',
    bio: 'Bio',
    experience_section: 'Experience',
    education: 'Education',
    languages: 'Languages',
    certifications: 'Certifications',
    portfolio: 'Portfolio',
    expectedSalary: 'Expected salary',
    availability: 'Availability',
    workPreference: 'Work preference',
    availabilityOptions: {
      immediate: 'Immediately',
      two_weeks: 'In two weeks',
      one_month: 'In a month',
      negotiable: 'Negotiable',
    },
    completion: {
      title: 'Complete your profile',
      subtitle: 'A complete profile gets noticed by more employers. Finish the steps below to reach 100%.',
      completeBody: 'Your profile is complete — you’re ready to stand out to employers.',
      progress: '{{done}} of {{total}} complete',
      remainingSection: '{{count}} steps left',
      completedSection: 'Completed ({{count}})',
      items: {
        title: 'Add your job title',
        bio: 'Write a short bio',
        location: 'Add your location',
        salary: 'Set your expected salary',
        skills: 'Add your skills',
        portfolio: 'Add a portfolio link',
        cv: 'Upload your CV',
        availability: 'Set your availability',
        workPreference: 'Set your work preference',
        experience: 'Add work experience',
        education: 'Add education',
        languages: 'Add a language',
        certifications: 'Add a certification',
      },
      hints: {
        title: 'e.g. Frontend Developer',
        bio: 'A sentence or two about you',
        location: 'Where you’re based',
        salary: 'Helps match you to the right roles',
        skills: 'Show employers what you can do',
        portfolio: 'Link your work or LinkedIn',
        cv: 'Let employers see your full CV',
        availability: 'When can you start?',
        workPreference: 'Full-time, remote, hybrid…',
        experience: 'Your past roles',
        education: 'Your degrees and studies',
        languages: 'Languages you speak',
        certifications: 'Courses and credentials',
      },
    },
    addExperience: 'Add experience',
    addEducation: 'Add education',
    addSkill: 'Add skill',
    addLanguage: 'Add language',
    addCertification: 'Add certification',
    jobTitle: 'Job title',
    company: 'Company',
    location: 'Location',
    startDate: 'Start date',
    endDate: 'End date',
    present: 'Present',
    description: 'Description',
    degree: 'Degree',
    school: 'Institution',
    fieldOfStudy: 'Field of study',
    applicants: 'applicants',
    posted: 'posted',
    ago: 'ago',
    daysAgo: '{{count}} days ago',
    requirements: 'Requirements',
    responsibilities: 'Responsibilities',
    benefits: 'Benefits',
    aboutCompany: 'About company',
    cvUploadSuccess: 'CV uploaded successfully',
    tapToUploadCv: 'Tap to upload your CV',
    cvUploadHint: 'PDF, DOC, DOCX supported\nMax 10MB',
  },
  profileCrud: {
    shared: {
      completed: 'Completed',
      selectDate: 'Select date',
      locationPlaceholder: 'Baku',
    },
    emptyStates: {
      experience: 'No experience added yet',
      education: 'No education added yet',
      languages: 'No languages added yet',
      certifications: 'No certifications added yet',
    },
    experience: {
      addTitle: 'Add experience',
      editTitle: 'Edit experience',
      jobTitlePlaceholder: 'Senior Frontend Developer',
      companyPlaceholder: 'Kapital Bank',
      statusLabel: 'Employment status',
      currentStatus: 'Current role',
      descriptionPlaceholder: 'Describe your impact, scope, and outcomes',
      highlightsLabel: 'Highlights',
      highlightPlaceholder: 'Add a measurable achievement',
    },
    education: {
      addTitle: 'Add education',
      editTitle: 'Edit education',
      degreePlaceholder: 'Bachelor',
      fieldOfStudyPlaceholder: 'Computer Science',
      institutionPlaceholder: 'Baku State University',
      statusLabel: 'Education status',
      currentStatus: 'Current study',
      descriptionPlaceholder: 'Add relevant academic details',
    },
    language: {
      addTitle: 'Add language',
      editTitle: 'Edit language',
      languageLabel: 'Language',
      languagePlaceholder: 'English',
      proficiencyLabel: 'Proficiency',
      proficiencyPlaceholder: 'Select level',
      levels: {
        beginner: 'Beginner',
        intermediate: 'Intermediate',
        advanced: 'Advanced',
        native: 'Native',
      },
    },
    certification: {
      addTitle: 'Add certification',
      editTitle: 'Edit certification',
      nameLabel: 'Certification name',
      namePlaceholder: 'AWS Solutions Architect',
      issuerLabel: 'Issuer',
      issuerPlaceholder: 'Amazon Web Services',
      issueDateLabel: 'Issue date',
      expiryDateLabel: 'Expiry date',
      expiryDatePlaceholder: 'Optional',
      credentialUrlLabel: 'Credential URL',
      credentialUrlPlaceholder: 'https://credential-link.com',
    },
  },
  apply: {
    sheetHint: 'Add an optional cover letter, then submit your application.',
    coverLetterLabel: 'Cover letter (optional)',
    writeWithAi: 'Write with AI',
    coverLetterPlaceholder: 'Introduce yourself to the employer...',
    answerQuestions: 'Answer a few questions',
    answerQuestionsHint: 'The employer asks applicants to answer these before applying.',
    yourAnswer: 'Your answer',
    answerRequired: 'Please answer all required questions.',
  },
  employer: {
    screeningQuestions: 'Screening questions (optional)',
    screeningQuestionsHint: 'Ask applicants to answer these when they apply.',
    addQuestion: 'Add a question',
    questionRequired: 'Required',
    questionOptional: 'Optional',
    screeningAnswers: 'Screening answers',
    dashboard: 'Dashboard',
    vacancies: 'Vacancies',
    applicants: 'Applicants',
    company: 'Company',
    settings: 'Settings',
    createVacancy: 'Create vacancy',
    editVacancy: 'Edit vacancy',
    activeVacancies: 'Active vacancies',
    totalApplicants: 'Total applicants',
    thisWeek: 'This week',
    thisMonth: 'This month',
    newApplicants: 'New applicants',
    pendingReview: 'Pending review',
    shortlisted: 'Shortlisted',
    hired: 'Hired',
    rejected: 'Rejected',
    companyProfile: 'Company profile',
    editCompany: 'Edit company',
    companyName: 'Company name',
    industry: 'Industry',
    employeeCount: 'Employees',
    website: 'Website',
    about: 'About',
    companyLogo: 'Company logo',
    vacancyTitle: 'Vacancy title',
    vacancyDescription: 'Description',
    salaryRange: 'Salary range',
    experienceRequired: 'Required experience',
    publishVacancy: 'Publish vacancy',
    saveDraft: 'Save as draft',
    viewApplicants: 'View applicants',
    review: 'Review',
    candidateReview: 'Candidate review',
    reviewNotes: 'Review notes',
    internalReviewOnly: 'Internal employer review only',
    notes: 'Notes',
    notesPlaceholder: 'Interview signals, concerns, next steps...',
    appliedOn: 'Applied {{date}}',
    markReviewed: 'Mark reviewed',
    accept: 'Accept',
    shortlist: 'Shortlist',
    reject: 'Reject',
    pause: 'Pause',
    activate: 'Activate',
    closeVacancy: 'Close',
    deleteVacancyConfirm: 'Delete "{{title}}"?',
    download_cv: 'Download CV',
    noVacancies: 'No vacancies',
    noVacanciesDesc: 'Create your first vacancy',
    noApplicants: 'No applicants',
    noApplicantsDesc: 'Wait for responses to your vacancies',
    analytics: 'Analytics',
    views: 'Views',
    responses: 'Responses',
    verification: 'Verification',
    verified: 'Verified',
    pending_verification: 'Pending verification',
    notVerified: 'Not verified',
    rating: 'Rating',
    details: 'Details',
    founded: 'Founded',
    candidateFallback: 'Candidate',
    verifiedDescription: 'Your company has been verified',
    verificationInProgressDescription: 'Verification is in progress',
    verificationRejectedDescription: 'Verification was rejected',
    verificationNotStartedDescription: 'Company is not verified yet',
    skillInputPlaceholder: 'Type a skill and press Add',
    vacancyTitlePlaceholder: 'e.g. Senior React Native Developer',
    onePerLine: 'One per line',
    companyNamePlaceholder: 'Company LLC',
    industryPlaceholder: 'Technology',
    aboutPlaceholder: 'About the company...',
    status: {
      active: 'Active',
      draft: 'Draft',
      pending: 'Pending review',
      pendingModeration: 'Pending moderation',
      reviewed: 'Reviewed',
      shortlisted: 'Shortlisted',
      rejected: 'Rejected',
      accepted: 'Accepted',
      paused: 'Paused',
      closed: 'Closed',
    },
  },
  notifications: {
    title: 'Notifications',
    empty: 'No notifications',
    emptyDesc: 'Updates will appear here',
    newApplication: 'New application',
    applicationUpdate: 'Application update',
    newJob: 'New vacancy',
    profileReminder: 'Profile reminder',
    markAllRead: 'Mark all as read',
    today: 'Today',
    earlier: 'Earlier',
  },
  settings: {
    title: 'Settings',
    contactSupport: 'Contact support',
    premium: 'Premium',
    supportPremiumTitle: 'Premium feature',
    supportPremiumMessage: 'Contacting support is available on paid plans. Upgrade to unlock priority support.',
    viewPlans: 'View plans',
    language: 'Language',
    theme: 'Appearance',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    notifications: 'Notifications',
    privacy: 'Privacy',
    help: 'Help',
    about: 'About',
    signOut: 'Sign out',
    version: 'Version',
    deleteAccount: 'Delete account',
    termsOfService: 'Terms of Service',
    privacyPolicy: 'Privacy Policy',
    legal: 'Legal',
    deleteAccountConfirmTitle: 'Delete your account?',
    deleteAccountConfirmMessage:
      'This action cannot be undone. All your data will be permanently deleted.',
    deleteAccountFinalTitle: 'Final confirmation',
    deleteAccountFinalMessage: 'Confirm to permanently delete your account.',
    deleteAccountSuccess: 'Your account has been deleted',
    accounts: 'Accounts',
    switchAccount: 'Switch account',
    addAccount: 'Add account',
    currentAccount: 'Current',
    switchFailed: 'Could not switch account. Please sign in again.',
    removeAccount: 'Remove account from list',
  },
  legal: {
    termsTitle: 'Terms of Service',
    privacyTitle: 'Privacy Policy',
    lastUpdated: 'Last updated: 2026',
    termsBody:
      'By using the AxtarIS platform you agree to these terms. The platform is designed to connect job seekers with employers.\n\nUsers must provide accurate information and use the platform only for lawful purposes. Prohibited content or abuse may result in account suspension.\n\nThis document is placeholder text and may change before the final legal version is published.',
    privacyBody:
      'AxtarIS respects your privacy. We collect only the data needed to provide the service — profile, CV and application details.\n\nYour data is not sold to third parties. CV files are stored securely and are only visible to employers you have applied to.\n\nYou can delete your account and data at any time. This document is placeholder text.',
  },
  ai: {
    generateWithAI: 'Generate with AI',
    generateHint: 'Auto-fill the description, requirements, and responsibilities from the job title.',
    upgradeTitle: 'Premium feature',
    jobDescriptionUpgrade: 'AI job descriptions are available on Pro and Premium plans. Upgrade to generate postings instantly.',
    titleRequiredForGeneration: 'Enter a job title first so AI has something to work with.',
    applicantRankingUpgrade: 'AI applicant ranking is a Premium feature. Upgrade to sort candidates by fit.',
    rankByFit: 'Rank by fit',
    clearRanking: 'Clear ranking',
    fitScore: 'Fit: {{score}}%',
    translate: 'Translate',
    showOriginal: 'Show original',
    translateUnavailable: 'Translation is unavailable right now.',
    assistant: 'AI Assistant',
    improveProfile: 'Improve profile',
    buildResume: 'Build resume',
    suggestSkills: 'Suggest skills',
    rewriteExperience: 'Rewrite experience',
    analyzingProfile: 'Analyzing profile...',
    suggestions: 'Suggestions',
    apply_suggestion: 'Apply suggestion',
    heroSubtitle: 'AI-powered suggestions to make your profile stand out',
    applied: 'Applied',
    suggestion: {
      bioTitle: 'Improve your profile description',
      bioDescription: 'A more detailed profile description attracts employers.',
      bioSuggested:
        'Experienced software engineer with a passion for building scalable applications. Skilled in modern web technologies with a track record of delivering high-quality solutions.',
      skillsTitle: 'Add more skills',
      skillsDescription: 'We recommend adding at least 5 skills.',
      experienceTitle: 'Add experience achievements',
      experienceDescription: 'Add at least 2-3 achievements to each work experience.',
      experienceTemplate:
        'Key achievements:\n• Led cross-functional team initiatives\n• Improved system performance by 30%\n• Mentored junior team members',
      profileTitle: 'Complete your profile',
      profileDescription:
        'Your profile is {{completeness}}% complete. A complete profile helps you get more job offers.',
    },
    fallback: {
      jobDescriptionWithCity:
        'We are looking for a {{title}} to join our team in {{city}}. You will work with {{skills}} and contribute to impactful projects.',
      jobDescription:
        'We are looking for a {{title}} to join our team. You will work with {{skills}} and contribute to impactful projects.',
      requirementExperience: 'Experience as a {{title}} or in a similar role',
      requirementProficiency: 'Proficiency in {{skills}}',
      requirementCommunication: 'Strong communication and teamwork skills',
      responsibilityDeliver: 'Deliver high-quality work as a {{title}}',
      responsibilityCollaborate: 'Collaborate with cross-functional teammates',
      responsibilityImprove: 'Continuously improve processes and outcomes',
      relevantTools: 'relevant tools',
      coverLetter:
        'Dear {{company}},\n\nI am excited to apply for the {{vacancyTitle}} role. As a {{candidateTitle}} with experience in {{skills}}, I am confident I can contribute meaningfully to your team. I would welcome the opportunity to discuss how my background fits your needs.\n\nBest regards,\n{{candidateName}}',
      hiringTeam: 'Hiring Team',
      genericProfessional: 'professional',
      myField: 'my field',
      experienceBulletPrefix: 'Spearheaded',
      matchingSkillsOne: '{{count}} matching skill: {{skills}}',
      matchingSkillsMany: '{{count}} matching skills: {{skills}}',
      noSkillOverlap: 'No direct skill overlap',
      resumeSummary:
        'Results-driven {{title}} with {{years}}+ years of experience. Proven track record in {{skills}}. Seeking to leverage expertise in a challenging new role.',
      resumeHighlightYears: '{{years}}+ years of professional experience',
      resumeHighlightSkills: 'Skilled in {{skills}}',
      resumeHighlightEducation: '{{degree}} from {{institution}}',
      resumeHighlightSelfTaught: 'Self-taught professional',
      resumeHighlightLanguages: '{{count}} languages spoken',
      resumeImprovedBio:
        'Dedicated {{title}} based in {{location}} with extensive experience in delivering high-impact solutions. Combining technical expertise with strong communication skills to drive business outcomes.',
      defaultLocation: 'Azerbaijan',
    },
  },
  moderation: {
    reportCta: 'Report',
    reportVacancyTitle: 'Report this vacancy',
    reportCompanyTitle: 'Report this company',
    reportProfileTitle: 'Report this profile',
    reportMessage: 'Why are you reporting this?',
    reportReasonScam: 'Scam or fake listing',
  },
  validation: {
    required: 'This field is required',
    invalidEmail: 'Enter a valid email address',
    passwordMin: 'Password must be at least 8 characters',
    passwordMatch: 'Passwords do not match',
    invalidPhone: 'Enter a valid phone number',
    fileTooLarge: 'File must be smaller than {{maxSize}}',
    salaryNumber: 'Salary must be a number',
    salaryRange: 'Minimum salary cannot exceed the maximum',
    dateRange: 'End date cannot be before the start date',
    invalidUrl: 'Enter a valid URL',
    otpCode: 'Enter the 6-digit code',
  },
  experienceLevel: {
    no_experience: 'No experience',
    junior: 'Junior',
    mid: 'Mid',
    senior: 'Senior',
    lead: 'Lead',
    executive: 'Executive',
  },
  errors: {
    generic: 'Something went wrong. Please try again.',
    duplicate: 'This already exists.',
    permission: 'You don’t have permission to do this.',
    network: 'Network error. Check your connection.',
    registrationFailed: 'Registration failed',
    signInFailed: 'Sign in failed',
    recoverySessionFailed: 'Recovery session could not be restored',
    unsupportedFileType: 'Unsupported file type',
    fileExceedsSizeLimit: 'Selected file exceeds the allowed size limit',
    fileReadFailed: 'Failed to read the selected file',
    userNotFound: 'User not found',
    candidateProfileNotFound: 'Candidate profile not found',
    vacancyNotFound: 'Vacancy not found',
    subscriptionNotFound: 'Subscription not found',
    subscriptionSummaryNotFound: 'Subscription summary not found',
    companyNotFound: 'Company not found',
    applicationNotFound: 'Application not found',
    companyAndCandidateRequired: 'Company and candidate are required',
    ratingRange: 'Rating must be between 1 and 5',
    emptyMessage: 'Message cannot be empty',
    missingImage: 'Image is missing',
    searchNameRequired: 'Search name is required',
    dailyLimitReached: 'Daily application limit reached for your current plan',
    saveToggleFailed: 'Could not update saved jobs. Please try again.',
    reportReasonRequired: 'Please choose a reason for the report',
  },
  subscription: {
    title: 'Subscription',
    currentPlan: 'Current plan',
    planOptions: 'Plans',
    comparison: 'Plan comparison',
    feature: 'Feature',
    upgrade: 'Upgrade',
    manage: 'Manage',
    managePlan: 'Manage plan',
    requestPlan: 'Request plan',
    requestPlanTitle: 'Request {{plan}}',
    requestPlanMessage: 'The AxtarIS team will contact you to set up this plan and arrange billing.',
    currentPlanCta: 'Current plan',
    choosePlan: 'Choose {{plan}}',
    planUpdated: 'Plan updated',
    popular: 'Popular',
    planLabel: '{{plan}} plan',
    unlimitedToday: 'Unlimited today',
    leftToday: '{{count}} left today',
    usedTodayOnly: 'Used today: {{count}}',
    usedTodayWithLimit: '{{used}}/{{limit}} applications used today',
    summary: {
      unlimitedWithVisibility: 'Unlimited applications today · {{visibility}}',
      remainingWithVisibility: '{{count}} applications left today · {{visibility}}',
    },
    upsell: {
      titleFree: 'Need more applications today?',
      bodyFree: 'Upgrade to Pro for 10 daily applications and priority placement in employers’ applicant lists.',
      titlePaid: 'Stay visible while you apply',
      bodyPaid: 'Track your daily quota and switch plans anytime to keep your profile competitive.',
    },
    catalogs: {
      candidate: {
        title: 'Job seeker plans',
        description: 'Get your application seen first, apply to more roles, and reach the AxtarIS team when it matters.',
        settingsDescription: 'Compare Standard, Pro, and Premium job seeker plans',
        plans: {
          free: {
            name: 'Standard',
            priceLabel: 'Free',
            visibilityLabel: 'Standard placement',
            highlights: [
              'Send up to 3 applications per day',
              'Standard placement in employers’ applicant lists',
              'Save jobs and message employers after you apply',
            ],
          },
          pro: {
            name: 'Pro',
            priceLabel: 'Free during beta',
            visibilityLabel: 'Priority in applicant lists',
            highlights: [
              'Send up to 10 applications per day',
              'Priority placement — your application appears above standard applicants',
              'A Pro badge on every application you send',
            ],
          },
          premium: {
            name: 'Premium',
            priceLabel: 'Free during beta',
            visibilityLabel: 'Top of applicant lists',
            highlights: [
              'Unlimited applications',
              'Top priority — you appear above Pro and standard applicants',
              'Direct support from the AxtarIS team',
            ],
          },
        },
        features: {
          price: {
            label: '30-day price',
            free: 'Free',
            pro: 'Free during beta',
            premium: 'Free during beta',
          },
          applications: {
            label: 'Daily applications',
            free: '3 per day',
            pro: '10 per day',
            premium: 'Unlimited',
          },
          cvVisibility: {
            label: 'Applicant priority',
            free: 'Standard',
            pro: 'Priority',
            premium: 'Top priority',
          },
          support: {
            label: 'Support',
            free: 'Standard',
            pro: 'Standard',
            premium: 'AxtarIS team',
          },
          directMessaging: {
            label: 'Employer messaging',
            free: 'After you apply',
            pro: 'After you apply',
            premium: 'After you apply',
          },
        },
      },
      employer: {
        title: 'Employer plans',
        description: 'Hire faster with priority support, hands-on help, and early access to promotion tools from the AxtarIS team.',
        settingsDescription: 'Compare Standard, Pro, and Premium employer plans',
        plans: {
          free: {
            name: 'Standard',
            priceLabel: 'Free',
            visibilityLabel: 'Standard vacancy placement',
            highlights: [
              'Post vacancies at no cost',
              'Manage applicants and message candidates',
            ],
          },
          pro: {
            name: 'Pro',
            priceLabel: 'Free during beta',
            visibilityLabel: 'Priority support & onboarding',
            highlights: [
              'Priority support and hiring setup help',
              'Early access to vacancy promotion tools as they launch',
              'Everything in Standard',
            ],
          },
          premium: {
            name: 'Premium',
            priceLabel: 'Free during beta',
            visibilityLabel: 'Dedicated VIP support',
            highlights: [
              'Dedicated VIP support from the AxtarIS team',
              'Managed candidate shortlists for your key roles',
              'Priority access to featured placement at launch',
            ],
          },
        },
        features: {
          price: {
            label: '30-day price',
            free: 'Free',
            pro: 'Free during beta',
            premium: 'Free during beta',
          },
          vacancyVisibility: {
            label: 'Vacancy promotion',
            free: 'Standard',
            pro: 'Early access',
            premium: 'Priority at launch',
          },
          vipSupport: {
            label: 'Support',
            free: 'Standard',
            pro: 'Priority',
            premium: 'Dedicated VIP',
          },
          candidateCuration: {
            label: 'Hiring assistance',
            free: 'Not included',
            pro: 'Setup help',
            premium: 'Managed shortlists',
          },
          directMessaging: {
            label: 'Candidate messaging',
            free: 'Included',
            pro: 'Included',
            premium: 'Included',
          },
        },
      },
    },
  },
  guest: {
    signInRequiredTitle: 'Account required',
    switchToEmployer: 'Switch to employer view',
    switchToCandidate: 'Switch to candidate view',
    signInRequiredMessage: 'Sign in or create an account to continue.',
    browsing: 'Guest mode',
    title: 'Sign in to your account',
    subtitle: 'Sign in to build a profile, apply, and save jobs.',
  },
  admin: {
    financeTab: 'Finance',
    financeTitle: 'Finance',
    financeSubtitle: 'Subscription revenue across the platform.',
    mrr: 'MRR',
    arr: 'ARR',
    arpu: 'ARPU',
    payingSubscribers: 'Paying subscribers',
    revenueByPlan: 'Revenue by plan',
    noRevenue: 'No paid subscriptions yet.',
    subscribersCount: '{{count}} subscribers',
    financeNote: 'Based on {{active}} active subscriptions. Estimated from subscription prices.',
    engagement: 'Engagement (30 days)',
    viewToApply: 'View → apply',
    evt: {
      vacancy_view: 'Job views',
      application_submit: 'Applications',
      message_sent: 'Messages sent',
      vacancy_publish: 'Vacancies published',
    },
    roleLabels: {
      candidate: 'Candidate',
      employer: 'Employer',
      admin: 'Admin',
    },
    moderationEntity: {
      vacancy: 'Vacancy',
      company: 'Company',
      user: 'User',
    },
    dash: {
      today: 'Today',
      activity: '14-day activity',
      users: 'Users',
      marketplace: 'Marketplace',
      engagement: 'Engagement',
      safety: 'Safety & moderation',
      revenue: 'Revenue',
      newUsers: 'New users',
      aiUses: 'AI uses',
      new7d: 'New users (7 days)',
      new30dSub: '30 days: {{count}}',
      pushDevices: 'Devices with push',
      inactive: 'Deactivated users',
      withCv: 'Candidates with CV',
      completenessSub: 'avg completeness {{pct}}%',
      vacancyBreakdownSub: '{{active}} active · {{draft}} draft · {{paused}} paused',
      totalViews: 'Vacancy views (total)',
      featuredSub: '{{count}} featured',
      applicationsByStatus: 'Applications by status',
      appsTodaySub: 'today: {{count}} · 7 days: {{count7}}',
      messages7d: 'Messages (7 days)',
      msgTodaySub: 'today: {{count}}',
      activeConvos: 'Active chats (7 days)',
      convosTotalSub: 'total: {{count}}',
      invitesSub: '{{accepted}} accepted · {{pending}} pending',
      profileViews30d: 'Profile views (30 days)',
      notifications7d: 'Notifications (7 days)',
      resolved7dSub: 'resolved (7 days): {{count}}',
      blockedPairs: 'Blocks',
      supportConvos: 'Support chats',
      aiUses30d: 'AI uses (30 days)',
      aiUsersSub: '{{count}} users',
      mrrSub: 'candidates {{c}} · employers {{e}}',
      payingSubs: 'Paying subscribers',
      payingSubsSub: '{{c}} candidates · {{e}} employers',
      notifType: {
        new_application: 'New application',
        application_update: 'Application update',
        new_message: 'New message',
        new_job_match: 'Job match',
        profile_reminder: 'Profile reminder',
        company_verification: 'Company verification',
        vacancy_moderation: 'Vacancy moderation',
        system: 'System',
      },
    },
    panel: 'Admin panel',
    dashboard: 'Dashboard',
    moderation: 'Moderation',
    users: 'Users',
    companies: 'Companies',
    overview: 'Overview',
    stats: {
      users: 'Users',
      candidates: 'Candidates',
      employers: 'Employers',
      admins: 'Admins',
      companies: 'Companies',
      verified: 'Verified',
      pendingVerification: 'Pending verification',
      vacancies: 'Vacancies',
      active: 'Active',
      pendingModeration: 'Pending moderation',
      applications: 'Applications',
      flags: 'Flags',
    },
    approve: 'Approve',
    reject: 'Reject',
    takedown: 'Take down',
    verify: 'Verify',
    unverify: 'Unverify',
    role: 'Role',
    makeAdmin: 'Make admin',
    makeCandidate: 'Make candidate',
    makeEmployer: 'Make employer',
    activate: 'Activate',
    deactivate: 'Deactivate',
    active: 'Active',
    inactive: 'Inactive',
    searchUsers: 'Search by name or email',
    reviewQueue: 'Moderation queue',
    noPending: 'Nothing to review',
    flagsTitle: 'Flags',
    noFlags: 'No flags',
    resolve: 'Resolve',
    dismiss: 'Dismiss',
  },
  match: {
    badge: '{{score}}% match',
    title: 'Your match',
    whyFit: 'Why you fit',
    strong: 'Strong match',
    good: 'Good match',
    fair: 'Partial match',
    low: 'Low match',
    reasons: {
      skills: '{{count}} matching skills',
      location: 'Location matches',
      workType: 'Work type matches your preference',
      salary: 'Salary fits your expectations',
      experience: 'Experience level matches',
    },
  },
  empty: {
    title: 'Nothing here',
    subtitle: 'No data available yet',
  },
} as const;
